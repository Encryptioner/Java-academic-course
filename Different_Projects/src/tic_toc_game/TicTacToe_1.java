...
/*public class Tic_Toc_1_4m_javasoucecode {
...
public class TicTacToe_1 implements ActionListener {
...
int wonNumber1 = 1, wonNumber2 = 1, wonNumber3 = 1;
    ...
    {   //Setting game properties and layout and sytle...
        ...
        pnlNewGame.setLayout(new GridLayout(4, 1, 0, 0));//(4, 1, 2, 6)) e h vgap,hgap er jonno last er ta dekha jai na, (3, 2, 0, 0)) te 6 ta content add kora lagbe
        pnlNorth.setLayout(new FlowLayout(FlowLayout.CENTER));
        pnlSouth.setLayout(new FlowLayout(FlowLayout.CENTER));

        pnlNorth.setBackground(new Color(color-120, color-120, color-120));//Menubar Jekhane ase
        pnlSouth.setBackground(new Color(color-120, color-120, color-120));//1st window te menubar bade purotai, baki window te majhe

        pnlTop.setBackground(new Color(color-120, color-120, color-120));//Menubar er niche
        pnlBottom.setBackground(new Color(color+20, color+20, color+20));
        ...
        pnlPlayingField.setBackground(Color.PINK);
        for(int i=1; i<=9; i++) {
        ...
        pnlSouth.add(lblTitle);//Not seen
...
//-------------------START OF ACTION PERFORMED CLASS-------------------------//
    public void actionPerformed(ActionEvent click) {
        ...
        for(int i=1; i<=9; i++) 
        {
            if(source == btnEmpty[i] && turn < 10) {//maximum 9 click in playing panel
            btnEmptyClicked = true;
                if(!(turn % 2 == 0))
                btnEmpty[i].setText("X");
                else
                btnEmpty[i].setText("O");
                btnEmpty[i].setEnabled(false);//if it is true player can change his mark
                pnlPlayingField.requestFocus();//Needs to determine who has won
                turn++;
            }
        }
        if(btnEmptyClicked) {
        ...
        if(source == mnuNewGame) {
            ...
            if(!inGame) {
                btnEmpty[wonNumber1].setBackground(new Color(120, 120, 120));//Darkest
                btnEmpty[wonNumber2].setBackground(new Color(220, 220, 220));
                btnEmpty[wonNumber3].setBackground(new Color(140, 140, 140));//Darker
                turn = 1;
                for(int i=1; i<10; i++) {
                    btnEmpty[i].setText("");
...
Conclusion: So basically it checks if it is equal to the btnEmpty is equal to each set of numbers
    ...
    {
        new TicTacToe_1();// Calling the class construtor.
    }
}
